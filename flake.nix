{
  description = "Nicolas Berbiche's poorly organized dotfiles and computer configuration";
  ...
  outputs =
    ...
    let
      ...
      nixpkgsFor = forAllPlatforms (
        ...
        let
          ...
          patchedNixpkgs = if platform == "x86_64-linux" then patchedNixpkgs' else nixpkgs;
        in
            ...
            ++ [
              {
                ...
                system.nixos.versionSuffix = ".${
                  lib.substring 0 8 (nixpkgs.lastModifiedDate or nixpkgs.lastModified or "19700101")
    ...
    in
    {
      lib =
        pkgs:
        (import ./top-level/lib.nix {
          lib = pkgs.lib;
          pkgs = pkgs;
        }).extend
          (_: _: { inherit (inputs.home-manager.lib) hm; });
      ...
      devShell = forAllPlatforms (
        platform:
        let
          pkgs = nixpkgsFor.${platform};
          sops = inputs.sops-nix.packages.${platform};
          NIX_CONF_DIR =
            (pkgs.writeTextDir "etc/nix.conf" ''

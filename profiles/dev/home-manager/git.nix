...
let
  ...
  mkFunction = x:
    let y = lib.concatStringsSep "; " (lib.splitString "\n" x);
    in {
      __toString = _: "!f(){ ${y}; }; f";
      unwrapped = y;
    };
...
in lib.mkMerge [
  (lib.mkIf (isLinux && (config.services.gnome-keyring.enable
    || osConfig.services.gnome.gnome-keyring.enable or false)) {
      ...
      settings = lib.mkMerge [
        {
          ...
          user.name = config.my.identity.name;
        ...
        (lib.mkIf config.programs.neovim.enable {
          mergetool.nvimdiff = {
            cmd =
              "${config.programs.neovim.finalPackage}/bin/nvim -d \"$LOCAL\" \"$REMOTE\" \"$MERGE\" -c 'DiffviewOpen'";
          };
          difftool.nvimdiff = {
            cmd =
              "${config.programs.neovim.finalPackage}/bin/nvim -d \"$LOCAL\" \"$REMOTE\" -c 'DiffviewOpen'";
          };
        ...
        {
          alias = lib.mapAttrs (_n: toString) rec {
            a = "add";
            an = "${a} --intent-to-add";
            aa = "${a} --all";
            au = "${a} --update";
            ...
            sl = lib.concatStrings [
              ...
              "%C(red)%h%C(reset) - %C(dim yellow)(%C(bold magenta)%gd%C(dim yellow))"
              "%C(reset) %<(70,trunc)%s %C(green)(%ci) %C(bold blue)<%an>%C(reset)"
              "'"
            ];
            # Clone aliases
            cl = mkFunction ''
            ...
            cmard = mkFunction ''
              git ${cmar} "$@" --date="''${GIT_COMMITTER_DATE:-'format:relative:now'}"'';
            ...
            # Prints one alias
            alias = mkFunction
              "git config --get alias.\"$1\" || echo 'alias not found'";
            ...
            remote-github = mkFunction ''
              git remote add "$1" git@github.com:"''${2%/*}"/"''${2#*/}".git'';
            rgb = remote-github;
          };
        }
      ];

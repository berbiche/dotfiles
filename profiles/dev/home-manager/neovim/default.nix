...
in {
  imports = [ ./utils.nix ./ui.nix ];

  home.packages = [ pkgs.fzf pkgs.neovim-remote ] ++ lib.optional
    (lib.myLib.supportedOn pkgs.stdenv.hostPlatform.system pkgs.neovide)
    pkgs.neovide;

  # programs.neovim.defaultEditor = true;
  home.sessionVariables = { EDITOR = shellAliases.nvim; };

  home.shellAliases = shellAliases;

  xdg.configFile = {
    "${luaConfigLocation}/cmp.lua".text = import ./cmp.lua.nix moduleArgs;
    "${luaConfigLocation}/dashboard.lua".text =
      import ./dashboard.lua.nix moduleArgs;
    "${luaConfigLocation}/filetree.lua".text =
      import ./filetree.lua.nix moduleArgs;
    "${luaConfigLocation}/keybinds.lua".text =
      import ./keybinds.lua.nix moduleArgs;
    "${luaConfigLocation}/lsp.lua".text = import ./lsp.lua.nix moduleArgs;
    "${luaConfigLocation}/motions.lua".text =
      import ./motions.lua.nix moduleArgs;
    "${luaConfigLocation}/neovide.lua".text =
      import ./neovide.lua.nix moduleArgs;
    "${luaConfigLocation}/settings.lua".text =
      import ./settings.lua.nix moduleArgs;
    "${luaConfigLocation}/telescope.lua".text =
      import ./telescope.lua.nix moduleArgs;
  };
  ...
  programs.neovim = {
    ...
    plugins = with pkgs.vimPlugins;
      lib.mkMerge [
        ...
        (lib.mkIf (config.profiles.dev.wakatime.enable) [
          (config.lib.neovim.lazy {
            plugin = vim-wakatime;
            type = "lua";
            config = ''
              -- WakaTime CLI path
              vim.g.wakatime_OverrideCommandPrefix = [[${pkgs.wakatime-cli}/bin/wakatime]]
